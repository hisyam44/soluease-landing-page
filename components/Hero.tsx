    ...
    <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-4xl mx-auto mb-12">
          <div className="inline-flex items-center space-x-2 bg-cyan-50 border border-cyan-100 px-4 py-2 rounded-full mb-6">
            ...
            <span className="text-sm font-semibold text-[#1d97c1] uppercase tracking-wider">
              Batam's Premier Software House
            </span>
          </div>
          ...
          <p className="text-xl text-slate-600 leading-relaxed mb-10 max-w-2xl mx-auto">
            Soluease transforms complex business challenges into intuitive digital solutions. 
            Located in the heart of Batam, we build high-performance software that feels like magic.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-20">
          {[
            { icon: Code2, title: 'Clean Architecture', desc: 'Maintainable codebases that scale as you grow.' },
            { icon: Rocket, title: 'Swift Deployment', desc: 'From concept to production in record time.' },
            { icon: ShieldCheck, title: 'Enterprise Security', desc: 'Batam-based reliability with global security standards.' }
          ].map((item, i) => (
            <div key={i} className="bg-white/50 backdrop-blur-sm p-8 rounded-3xl border border-slate-100 hover:border-[#1d97c1]/30 transition-all hover:shadow-xl group">
              <div className="w-14 h-14 bg-cyan-50 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-[#1d97c1] transition-colors">
                <item.icon className="w-7 h-7 text-[#1d97c1] group-hover:text-white transition-colors" />
              </div>
